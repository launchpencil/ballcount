...
if 'df10' in st.session_state:
    straight = st.number_input("ストレート", value=math.floor(st.session_state.df10.loc["球数", "ストレート"]))
    curve = st.number_input("カーブ", value=math.floor(st.session_state.df10.loc["球数", "カーブ"]))
    slider = st.number_input("スライダー", value=math.floor(st.session_state.df10.loc["球数", "スライダー"]))
    changeup = st.number_input("チェンジアップ", value=math.floor(st.session_state.df10.loc["球数", "チェンジアップ"]))
    fork = st.number_input("フォーク", value=math.floor(st.session_state.df10.loc["球数", "フォーク"]))
    cutball = st.number_input("カットボール", value=math.floor(st.session_state.df10.loc["球数", "カットボール"]))
    two = st.number_input("ツーシーム", value=math.floor(st.session_state.df10.loc["球数", "ツーシーム"]))
    sinka = st.number_input("シンカー", value=math.floor(st.session_state.df10.loc["球数", "シンカー"]))
    shoot = st.number_input("シュート", value=math.floor(st.session_state.df10.loc["球数", "シュート"]))
...
if sumnorun != 0:
    ...
    df10 = pd.DataFrame(data, index=["球数", "割合"])
    st.dataframe(df10.T)
